...
void* get(node_t* head_list, int index) { 
  ...
  //    return 0;
  //  }   
   if (index >= length_list(head_list) || index < 0) {
     return NULL;
   }
   int counter = 0;
   node_t* curr = head_list;
...
int remove_element(node_t **head_list, void *to_remove, size_t size) {
  ...
  while (curr) {
    if (!memcmp(curr->data, to_remove, size)) {  // We've found the element to remove
      
      // Sets the previous pointer of the next node equal to the previous pointer of the current node
      if (curr->next) {
        curr->next->prev = curr->prev;
      }
      // Case here the element to remove is the first item in the linked list
      if (curr == *head_list) {
        *head_list = curr->next;
      } else {
        // Sets the next pointer of the previous node equal to the next pointer of the current node
